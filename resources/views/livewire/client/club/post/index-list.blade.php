                        ...
                        <ul class="category-list ">
                            @foreach($categories as $item)
                                <li class="d-flex justify-content-between">
                                    <div>
                                        <input type="checkbox" value="{{ $item->id }}" class="" style="width: 20px; height: 20px; accent-color: green; opacity: 1;" wire:model.live="selectedCategories">
                                        <a href="javascript:void(0)" class="ms-5" style="font-size: 16px; line-height: 20px">{{ $item->name }}</a>
                                    </div>
                                    <span class="count">({{ $item->posts->where('status', 'published')->count() }})</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="inner">
                        <ul>
                            @foreach($postFamous as $item)
                                <li>
                                    <a class="d-block" href="{{route('client.club.post-detail',['id'=>$club->id, 'slug'=>$item->slug])}}">
                                        {{$item->title}}
                                    </a>
                            ...
                            <div class="rainbow-card undefined">
                                <div class="inner">
                                    ...
                                    <div class="content" style="height: 130px">
                                        ...
                                        <h4 class="title"><a href="{{route('client.club.post-detail',['id'=>$club->id, 'slug'=>$post->slug])}}">{{$post->title}}</a></h4>
                                    </div>
                                </div>
                            </div>
                        </div>
