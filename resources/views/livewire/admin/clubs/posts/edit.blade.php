            ...
            <div class="card-body">
                ...
                <div class="form-group mt-2">
                    ...
                    <div class="mt-2">
                        ...
                        @endif
                        @if ($oldThumbnail && $oldThumbnail!=1)
    ...
    <div class="col-md-3">
        <div class="card">
            ...
            <div class="card-body d-flex flex-wrap align-items-center gap-1">
                @if($oldStatus == App\Enums\StatusPost::draft->name)
                    <button wire:click="updateDraft" class="btn btn-success" type="submit"><i class="ph-file"></i>Lưu bản nháp</button>
                @endif
                @if(Auth::user()->hasPermissonClub('Quản lý bài viết', $this->club_id))
                    @if($oldStatus != App\Enums\StatusPost::rejected->name)
                        <button wire:click="update" class="btn btn-primary" type="submit"><i class="ph-floppy-disk"></i>@if($oldStatus == App\Enums\StatusPost::draft->name || $oldStatus == App\Enums\StatusPost::pending->name || $oldStatus == App\Enums\StatusPost::scheduled->name) Đăng ngay @else Cập nhật @endif </button>
                        @if($oldStatus != App\Enums\StatusPost::published->name && $oldStatus != App\Enums\StatusPost::private->name)
                        ...
                        @endif
                        @if($oldStatus == App\Enums\StatusPost::published->name)
                            <button wire:click="privatePost" class="btn btn-danger" type="submit"><i class="ph-user-focus"></i>Chế độ riêng tư</button>
                        @endif
                        @if($oldStatus == App\Enums\StatusPost::private->name)
                            <button wire:click="publicPost" class="btn btn-info" type="submit"><i class="ph-user-focus"></i>Chế độ công khai</button>
                        @endif
                    @endif
                    @if($oldStatus == App\Enums\StatusPost::pending->name)<button wire:click="reject" class="btn btn-danger" type="submit"><i class="ph-x"></i>Từ chối</button>@endif
                @elseif(Auth::user()->hasPermissonClub('Tạo bài viết mới', $this->club_id))
                    <button wire:click="updateNormal" class="btn btn-primary" type="submit"><i class="ph-paper-plane-right"></i>@if($oldStatus == App\Enums\StatusPost::draft->name) Gửi duyệt bài @else Gửi duyệt lại @endif  </button>
                @endif
